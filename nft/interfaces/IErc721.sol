...
interface IErc721 {
    ...
    event Approval(
     ...
     event ApprovalForAll(
        address indexed _owner, address indexed _operator, bool _approvedsafeTransferFrom

    function safeTransferFrom(
        address _from, address _to, uint256 _tokenId, bytes calldata _data ) external;
    ...
    function approve(
        address _approved,  uint256 _tokenId ) external;

    function setApprovalForAll(
        address _operator,  bool _approved ) external;

    function balanceOf(
        address _owner)  external view returns (uint256);
